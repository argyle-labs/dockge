...
//! own statically-linked tool inventory (`dockge.*` endpoint CRUD + stack ops)
...
/// tools alongside the plugin's. The plugin exposes only its own `dockge.*`
...
const TOOL_PREFIX: &str = "dockge.";

/// The plugin's own tool surface: `tool_manifest_json()` filtered to the
/// `dockge.*` namespace. Shared by `manifest()` (serialized back out) and
...
/// exposes (HTTP-only endpoint CRUD + stack ops) needs no host-injected
